...
final class Review_app_userRepository
{
    ...
    {
        $query = 'INSERT INTO `review_app_user` (`id`, `deviceId`, `type`, `os`, `deleted_at`, `created_at`, `updated_at`) VALUES (:id, :deviceId, :type, :os, :deleted_at, :created_at, :updated_at)';
        $statement = $this->getDb()->prepare($query);
        $statement->bindParam('id', $review_app_user->id);
	$statement->bindParam('deviceId', $review_app_user->deviceId);
	$statement->bindParam('type', $review_app_user->type);
    ...
    public function update(object $review_app_user, object $data)
    {
        ...
        if (isset($data->type)) { $review_app_user->type = $data->type; }
        ...
        $query = 'UPDATE `review_app_user` SET `deviceId` = :deviceId, `type` = :type, `os` = :os, `deleted_at` = :deleted_at, `created_at` = :created_at, `updated_at` = :updated_at WHERE `id` = :id';
        $statement = $this->getDb()->prepare($query);
        $statement->bindParam('id', $review_app_user->id);
	$statement->bindParam('deviceId', $review_app_user->deviceId);
	$statement->bindParam('type', $review_app_user->type);
